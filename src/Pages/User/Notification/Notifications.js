...
const Notifications = () => {
    ...
    const fetchNotifications = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/user/notification/getnotifications",
          {
            headers: {
  ...
  const handlePreview = async (filename) => {
    try {
      const authToken = localStorage.getItem("token");
      const response = await axios.get(
        "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/user/notification/previewnotification",
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
  ...
  const handleDownload = async (filename) => {
    try {
      const authToken = localStorage.getItem("token");
      const response = await axios.get(
        "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/user/notification/download",
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
