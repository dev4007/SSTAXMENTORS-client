...
const PaymentSettings = () => {
    ...
    const fetchPaymentImage = async () => {
      try {
        const response = await axios.get(
          "https://www.sstaxmentors.com/admin/PaymentQR"
        );
  ...
  const fetchPaymentImagePreview = async (filename) => {
    try {
      const response = await axios.get(
        "https://www.sstaxmentors.com/admin/getPaymentQRImage",
        {
          responseType: "blob",
  ...
  const handlePaymentDelete = async () => {
    try {
      await axios.post("https://www.sstaxmentors.com/admin/deletePaymentQR");
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://www.sstaxmentors.com/admin/addPaymentQRImage",
        formData,
        {
          headers: {
      ...
      const response = await axios.get("https://www.sstaxmentors.com/admin/PaymentQR");
      if (response.data.paymentImages.length > 0) {
        setPaymentImage(response.data.paymentImages[0]);
      }
    } catch (error) {
