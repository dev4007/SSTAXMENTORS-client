...
const PaymentSettings = () => {
    ...
    const fetchPaymentImage = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/admin/PaymentQR`
        );
  ...
  const fetchPaymentImagePreview = async (filename) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/admin/getPaymentQRImage`,
        {
          responseType: "blob",
  ...
  const handlePaymentDelete = async () => {
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/admin/deletePaymentQR`);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_URL}/admin/addPaymentQRImage`,
        formData,
        {
          headers: {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/admin/PaymentQR`);
      if (response.data.paymentImages.length > 0) {
        setPaymentImage(response.data.paymentImages[0]);
      }
    } catch (error) {
