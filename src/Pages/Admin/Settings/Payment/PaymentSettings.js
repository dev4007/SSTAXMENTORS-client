...
const PaymentSettings = () => {
    ...
    const fetchPaymentImage = async () => {
      try {
        const response = await axios.get(
          "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/PaymentQR"
        );
  ...
  const fetchPaymentImagePreview = async (filename) => {
    try {
      const response = await axios.get(
        "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/getPaymentQRImage",
        {
          responseType: "blob",
  ...
  const handlePaymentDelete = async () => {
    try {
      await axios.post("https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/deletePaymentQR");
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/addPaymentQRImage",
        formData,
        {
          headers: {
      ...
      const response = await axios.get("https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/PaymentQR");
      if (response.data.paymentImages.length > 0) {
        setPaymentImage(response.data.paymentImages[0]);
      }
    } catch (error) {
