...
const AdminBannerSettings = () => {
    ...
    const fetchImages = async () => {
      try {
        ...
        const response = await axios.get(
          "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/banner/getBannerSettingsImages",
          {
            headers: {
  ...
  const handleAddLoginImage = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/banner/addBannerImage", formData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleImageClick = async (imageName, title) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/banner/getBannerImage/${imageName}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const confirmDeleteImage = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/banner/deleteBannerImage",
