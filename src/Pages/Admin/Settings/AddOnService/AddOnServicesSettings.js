...
const AddOnServices = () => {
  ...
  const handleAddAddOnService = async () => {
    try {
      await axios.post("https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/addonservice/addNewAddOnService", {
  ...
  const handleRemoveAddOnService = async (addOnServiceId) => {
    try {
      await axios.delete(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/addonservice/deleteAddOnService/${addOnServiceId}`
      );
      setErrorMessage("");
      // Refresh add-on services list
      const token = localStorage.getItem("token");
  ...
  const handleRemoveSubService = async (addOnServiceId, subServiceIndex) => {
    ...
    try {
      await axios.delete(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/addonservice/deleteSubService/${addOnServiceId}/${subServiceIndex}`
      );
      setErrorMessage("");
      // Refresh add-on services list
      const token = localStorage.getItem("token");
  ...
  const handleSubmitSubServices = async (addOnServiceId) => {
    try {
      await axios.post(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/addonservice/addSubServices/${addOnServiceId}`,
