...
const SupportTicket = () => {
  ...
  const fetchSupportTicketData = async (client) => {
    try {
      ...
      const response = await axios.get(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/supportticket/getClientsSupportTickets?clientEmail=${selectedClient}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleResolve = async (ticketId) => {
    try {
      ...
      await axios.patch(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/supportticket/resolveSupportTicket/${ticketId}`,
        null,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handlePreview = async (fileId, filename) => {
    try {
      ...
      console.log(fileId);
      const response = await axios.get(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/user/supportticket/previewSupportTicket/${fileId}`,
        {
          headers: {
  ...
  const handleDownload = async (fileId, filename) => {
    try {
      const authToken = localStorage.getItem("token");
      const response = await axios.get(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/user/supportticket/downloadSupportTicket/${fileId}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
  ...
  const handleView = async (index, ticketId) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/supportticket/getSupportTicketUsingTicketid/${ticketId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
