...
const CreatePaymentBill = () => {
        ...
        try {
          ...
          if (selectedService === "IT") {
            ...
            const token =
              localStorage.getItem("token") 
            await axios.post(
              `${process.env.REACT_APP_API_URL}/admin/document/gstreturns/sendGSTnotice`,
              dataForBackend,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
  ...
  //       const response = await axios.get(
  //        `${process.env.REACT_APP_API_URL}/admin/getCompanyNameOnlyDetailsAdmin/${selectedCompany}`
