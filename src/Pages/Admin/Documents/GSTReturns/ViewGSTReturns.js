...
const ViewGSTReturns = () => {
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        setLoading(true);
        const response = await axios.get(
          "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/settings/gstreturns/getGSTReturnsFields"
        );
  ...
  const fetchGSTReturnsData = async (client) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await axios.get(
        "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/document/gstreturns/getGSTReturnsAdmin",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleConfirmDelete = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/document/gstreturns/deleteGSTReturnAdmin",
  ...
  const handlePreview = async (filename) => {
    try {
      ...
      const response = await axios.get(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/user/document/gstreturns/previewGSTReturns/${filename}`,
        {
          headers: {
  ...
  const handleDownload = async (filename) => {
    try {
      const authToken = localStorage.getItem("token");
      const response = await axios.get(
        `https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/user/document/gstreturns/downloadGSTReturns/${filename}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
