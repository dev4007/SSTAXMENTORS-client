...
const RocFilings = () => {
  ...
  const fetchRocFilingsData = async (client) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/admin/document/rocfilings/getROCFilingsAdmin`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleConfirmDelete = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${process.env.REACT_APP_API_URL}/admin/document/rocfilings/deleteROCFilingAdmin`,
