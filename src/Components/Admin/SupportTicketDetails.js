...
const SupportTicketDetails = () => {
  ...
  const fetchTicketDetails = async () => {
    try {
      ...
      const response = await axios.get(
       `${process.env.REACT_APP_API_URL}/admin/getSupportTicketUsingTicketid/${ticketId}`,
        {
          headers: {
  ...
  const handlePreview = async (fileId) => {
    try {
      ...
      const response = await axios.get(
       `${process.env.REACT_APP_API_URL}/admin/previewSupportTicketFile/${fileId}`,
        {
          headers: {
  ...
  const handleDownload = async (fileId) => {
    try {
      const authToken = localStorage.getItem("token");
      const response = await axios.get(
       `${process.env.REACT_APP_API_URL}/admin/downloadSupportTicketFile/${fileId}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
