...
function AdminEmailComponent() {
    ...
    async function fetchEmails() {
      try {
        const response = await axios.get(
          "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/getAdminsEmail"
        );
  ...
  const handleSubmit = async () => {
    try {
      await axios.post("https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/emails", {
        email: newEmail,
    ...
    if (selected !== selectedEmail) {
      try {
        const response = await axios.post(
          "https://sstaxmentors-dev4s-projects-8f80d394.vercel.app/admin/selectAdminsEmail",
          {
            email: selected,
          }
        );
